...
| Quality gating (pre-accept) | The docs expect review-time validation before acceptance | Lifecycle commands now have permission and transition constraints, but a unified pre-accept quality gate is not fully closed yet | OpenSquilla user/author docs；[SkillCommands.cs](../src/OpenClaw.Cli/SkillCommands.cs#L424)；[SkillCommands.cs](../src/OpenClaw.Cli/SkillCommands.cs#L528) | Partial |
| Stable catalog | The docs describe a stable built-in MetaSkill catalog | Generic skill loading exists, but there is no equally productized catalog story yet | OpenSquilla user docs；[SkillLoader.cs](../src/OpenClaw.Core/Skills/SkillLoader.cs#L1) | Partial |
